...
import { detectAttendanceAnomalies } from "./aiAnomalyDetector";
import { isEmailAlertsConfigured, sendParentAlert } from "./emailAlerts";
import "./App.css";
...
const PAGE_META = {
  ...
  anomalies: {
    ...
    description: "Tune sensitivity, inspect model-detected outliers, and mark items resolved.",
  },
  students: {
    badge: "Directory",
...
export default function App() {
  ...
  const notifiedAnomalyIdsRef = useRef(new Set());
  ...
  useEffect(() => {
    if (!isEmailAlertsConfigured) return;
    if (!attendanceRecords.length) return;

    const detection = detectAttendanceAnomalies(attendanceRecords, scheduleConfig);
    const pending = detection.anomalies.filter(
      (anomaly) =>
        anomaly.guardianEmail &&
        (anomaly.severity === "high" || anomaly.severity === "medium") &&
        !notifiedAnomalyIdsRef.current.has(anomaly.id)
    );
      ...
      for (const anomaly of pending) {
        try {
          await sendParentAlert({
            toEmail: anomaly.guardianEmail,
            studentName: anomaly.studentName,
            studentId: anomaly.studentId,
            severity: anomaly.severity,
            date: anomaly.date,
            message: anomaly.message,
            strand: anomaly.strand,
            timeIn: anomaly.timeIn,
            timeOut: anomaly.timeOut,
          });
          notifiedAnomalyIdsRef.current.add(anomaly.id);
        } catch (error) {
    ...
    sendAlerts();
  }, [attendanceRecords, scheduleConfig]);
    ...
    <div className="app-container">
      <nav className="navbar">
        ...
        <div className="nav-links">
          ...
          <button onClick={() => setTab("anomalies")} className={tab === "anomalies" ? "active" : ""}>
            Anomalies
          </button>
      ...
      <main className="main-content">
        ...
        {tab === "anomalies" && (
          <AnomalyDashboard attendanceRecords={attendanceRecords} scheduleConfig={scheduleConfig} />
        )}
        {tab === "students" && (
          <AllStudents
            students={students}
